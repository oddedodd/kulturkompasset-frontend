...
export default function AktueltArticlesGrid({
  initialArticles,
  pageSize = 9,
}: AktueltArticlesGridProps) {
  ...
  const fetchPage = useCallback(
    async ({ offset, append }: { offset: number; append: boolean }) => {
      ...
      try {
        const response = await fetch(`/api/aktuelt?${params.toString()}`);
        if (!response.ok) throw new Error("Unable to fetch aktuelt articles");
        ...
        if (requestId === requestIdRef.current) {
          setError("Kunne ikke hente artikler akkurat nå.");
        }
      }
    },
      ...
      {articles.length === 0 && !isRefreshing ? (
        <section className="mx-auto mt-6 w-full max-w-6xl bg-gray-100 px-6 py-16 text-center text-black/70">
          {hasActiveFilters
            ...
            : "Ingen artikler i Aktuelt akkurat nå."}
        </section>
      ) : null}
              ...
              >
                ...
                <div className="pointer-events-none relative z-20 flex h-full flex-col justify-end p-5 text-white sm:p-6">
                  ...
                  <p className="mt-3 line-clamp-4 text-sm leading-relaxed text-white/90">
                    {article.excerpt?.trim() || "Les saken i Aktuelt."}
                  </p>
                </div>
              </article>
              );
